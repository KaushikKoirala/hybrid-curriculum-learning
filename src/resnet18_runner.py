...
def do_training_loop(model, cfg, train_loader, gaussian_loader, val_loader, criterion, criterion_eval, optimizer, scheduler, mixup_fn, run_id):
    ...
    run = wandb.init(
        name = f"resnet18-{dataset_name}-{run_id}",
        reinit = True,
        project = "idl-project",
        config = cfg
    )
...
def main():
    ...
    parser.add_argument('-d', '--dataset', type=str, default='cifar', help='Dataset: cifar or imagenet')
    ...
    if args.dataset.lower() == 'cifar':
        ...
        cfg = get_resnet18_imagenet_config(run_id=run_logical_id, lerac_epochs=args.lerac_epochs, blur_epochs=args.blur_epochs, eta_min=args.eta_min)
